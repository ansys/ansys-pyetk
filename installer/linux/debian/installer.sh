...
if [ $dependencies_available = true ]; then
    ...
    if [ $available -lt 1 ]
    then
        echo -e "# Add alias for Ansys Template Toolkit \nalias ansys_pyetk=~/.local/opt/ansys_pyetk/ansys_pyetk" >> ~/.bashrc
    fi
    ...
    if [ "$user_selection" = "Y" ]; then
        ...
        if [ $root_check -eq 0 ]; then
            ...
            if [ $dependencies_available = false ]; then
                ...
                dpkg -x ./ansys_pyetk.deb ${HOME}/.local
                ./postInstallScript.sh
                available=$(cat ~/.bashrc | grep -zoP "# Add alias for Ansys Template Toolkit \nalias ansys_pyetk=~/.local/opt/ansys_pyetk/ansys_pyetk" | wc -l)
                echo $available
                if [ $available -lt 1 ]
                then
                    echo -e "# Add alias for Ansys Template Toolkit \nalias ansys_pyetk=~/.local/opt/ansys_pyetk/ansys_pyetk" >> ~/.bashrc
